...
type PropertySeed = Omit<PropertyLocation, "ownerId" | "houses" | "hasHotel">;
type RailroadSeed = Omit<RailroadLocation, "ownerId">;
type UtilitySeed = Omit<UtilityLocation, "ownerId">;
type NonOwnableLocation =
  | StartLocation
  | JailLocation
  | GoToJailLocation
  | FreeParkingLocation
...
type LocationSeed = PropertySeed | RailroadSeed | UtilitySeed | NonOwnableLocation;
...
function createLocation(locationDef: LocationSeed): BoardLocation {
  if (locationDef.type === "property") {
    return { ...locationDef, ownerId: null, houses: 0, hasHotel: false };
  }
  if (locationDef.type === "railroad" || locationDef.type === "utility") {
    return { ...locationDef, ownerId: null };
  }
  return { ...locationDef };
}
